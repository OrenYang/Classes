function Burger_KS_IMEXRKCB3C_PS_3R
...
for k=1:Tmax/dt
  for rk=1:3 %%%%%%%%%%%%%%%%%%%%% ALL RK SUBSTEPS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if (rk == 1)
      ...
      rhat = uhat+abim(rk)*dt*zhat+abex(rk)*dt*rhat;
    end
    zhat = Aop.*rhat./(1-aim(rk)*dt*Aop);
    rhat = rhat+aim(rk)*dt.*zhat;
    rhat(fix(N/3)+1:end) = 0;   %Dealias
    r = RC_RFFTinv(rhat,N); r = -1/2*r.*r; rhat = i*kx.*RC_RFFT(r,N);
    uhat = uhat+b(rk)*dt.*zhat+b(rk)*dt.*rhat;
  end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END OF RK LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end % function Burger_KS_IMEXRKCB3C_PS_3R
